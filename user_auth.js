(function () {
    ...
    const GOOGLE_REDIRECT_URI = window.FX_GOOGLE_REDIRECT_URI || 'https://earthman1976.github.io/FX-website/';
    ...
    const EXPECTED_GITHUB_ORIGIN = 'https://earthman1976.github.io';
    const REGISTRATION_SOURCE = 'GitHub_Production';
    ...
    const I18N = {
        zh: {
            ...
            errClientId: 'Google Client ID 格式錯誤，請使用 .apps.googleusercontent.com 結尾的 Client ID。',
        ...
        en: {
            ...
            errClientId: 'Google Client ID is invalid. It must end with .apps.googleusercontent.com.',
    ...
    function isValidGoogleClientId(value) {
        ...
        return /^[0-9]+-[a-z0-9-]+\.apps\.googleusercontent\.com$/i.test(normalized);
    }
    ...
    function loadScript(src, id) {
        return new Promise((resolve, reject) => {
            ...
            script.onerror = (event) => {
                const errorCode = `sdk_load_failed:${id || 'unknown'}:${src}`;
                alert(`Google SDK 載入失敗 (${errorCode})`);
                reject(new Error(errorCode, { cause: event }));
            };
            document.head.appendChild(script);
        });
    }

    function verifyRuntimeOrigin() {
        const currentOrigin = window.location.origin || '';
        if (!currentOrigin || !currentOrigin.startsWith('https://')) {
            console.warn('[FX Member Auth] Third-party sign-in works best under HTTPS origin.');
            return;
        }
        if (EXPECTED_GITHUB_ORIGIN && currentOrigin !== EXPECTED_GITHUB_ORIGIN) {
            console.warn(`[FX Member Auth] Origin mismatch. Current: ${currentOrigin}, Expected: ${EXPECTED_GITHUB_ORIGIN}`);
        }
        const currentHref = `${window.location.origin}${window.location.pathname}`;
        if (GOOGLE_REDIRECT_URI && !GOOGLE_REDIRECT_URI.startsWith(currentOrigin)) {
            console.warn(`[FX Member Auth] Google redirect URI may mismatch. Current page: ${currentHref}, Configured redirect: ${GOOGLE_REDIRECT_URI}`);
        }
    }
    ...
    async function ensureGoogleSDK() {
        ...
        if (!isValidGoogleClientId(GOOGLE_CLIENT_ID)) throw new Error('google_client_id_invalid');
    ...
    async function getGoogleProfile() {
        ...
        return new Promise((resolve, reject) => {
            ...
            googleTokenClient.requestAccessToken({ prompt: 'consent', redirect_uri: GOOGLE_REDIRECT_URI });
        });
    }
        ...
        try {
            ...
            if (errEl) {
                ...
                if (e?.message === 'missing_email') {
                    errEl.textContent = dict.errMissingEmail;
                } else if (e?.message === 'google_client_id_invalid') {
                    errEl.textContent = dict.errClientId;
                } else {
                    errEl.textContent = dict.errProvider;
                }
